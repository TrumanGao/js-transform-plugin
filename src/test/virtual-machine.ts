...
const Variable = require('./engine/variable.ts');
...
class VirtualMachine extends EventEmitter {
    constructor () {
         ...
         * AI智能模块
         */
        this.runtime.on(Runtime.ACTIVATE_CAMERA, (...args) => {
            this.emit(Runtime.ACTIVATE_CAMERA, args);
        });
        this.runtime.on(Runtime.ERROR_MESSAGE, args => {
            this.emit(Runtime.ERROR_MESSAGE, args);
        });
        this.runtime.on(Runtime.ACTIVATE_CALORIE, (...args) => {
            this.emit(Runtime.ACTIVATE_CALORIE, args);
        });
        this.runtime.on(Runtime.ACTIVATE_WEATHER, args => {
            this.emit(Runtime.ACTIVATE_WEATHER, args);
        });

        // 更新gui硬件积木库
        this.runtime.on(Runtime.UPDATE_SELECTED_DEVICE, (...args) => {
            this.emit(Runtime.UPDATE_SELECTED_DEVICE, args);
        });
        ...
        this.runtime.on(Runtime.PROJECT_RUN_STOP, type => {
            this.emit(Runtime.PROJECT_RUN_STOP, type);
        });
        ...
        this.runtime.on(Runtime.PERIPHERAL_CONNECTION_INVALID, data => {
            this.emit(Runtime.PERIPHERAL_CONNECTION_INVALID, data);
        });
        ...
        this.runtime.on(Runtime.PERIPHERAL_REALTIME_UPLOADED, data =>
            this.emit(Runtime.PERIPHERAL_REALTIME_UPLOADED, data),
        );
        ...
        this.runtime.on(Runtime.PERIPHERAL_RECIVE_DATA, data => this.emit(Runtime.PERIPHERAL_RECIVE_DATA, data));
        this.runtime.on(Runtime.PERIPHERAL_UPLOAD_STDOUT, info => this.emit(Runtime.PERIPHERAL_UPLOAD_STDOUT, info));
        this.runtime.on(Runtime.PERIPHERAL_UPLOAD_ERROR, info => this.emit(Runtime.PERIPHERAL_UPLOAD_ERROR, info));
        this.runtime.on(Runtime.PERIPHERAL_UPLOAD_SUCCESS, () => this.emit(Runtime.PERIPHERAL_UPLOAD_SUCCESS));
        ...
        // 改变zoom
        this.runtime.on(Runtime.ZOOM_CHANGE, zoom => {
            this.emit(Runtime.ZOOM_CHANGE, zoom);
        });

        this.extensionManager = new ExtensionManager(this.runtime);

        // 手动触发的帽子事件，需要进行检测
        this._checkHatsArray = [
    ...
    start () {
        ...
        // 添加逻辑：如果有硬件，且实时模式已经连接，需要检测固件升级
        const arduinoUno =
            this.runtime.peripheralExtensions && this.runtime._device
                ? this.runtime.peripheralExtensions[this.runtime._device]
                : null;
        if (arduinoUno && arduinoUno._startHeartbeat && arduinoUno.isReady && arduinoUno.isReady()) {
            arduinoUno._startHeartbeat();
        }
    }
    ...
    disconnectPeripheral (extensionId) {
        console.log('disconnectPeripheral:', extensionId);
    ...
    loadProject (input) {
        ...
        const validationPromise = new Promise((resolve, reject) => {
            ...
            validate(input, false, (error, res) => {
                // console.log('scratch-error:', error, res);
                if (error) {
                    return reject(error);
                }
                console.log('scratch-parser:', res);
                resolve(res);
            });
        }).catch(error => {
    ...
    fakeloadProject (input) {
        ...
        const validationPromise = new Promise((resolve, reject) => {
            ...
            validate(input, false, (error, res) => {
                ...
                console.log('fakeloadProject:', res);
                resolve(res);
            });
        });
        ...
        return validationPromise.then(
            validatedInput => {
                sb3.deserialize(validatedInput[0], that.runtime, validatedInput[1]);
            },
    ...
    // 检测作品完成状况
    async checkProject (inputList, step) {
        console.log('codeCheck: 1', inputList, step);
        // 解析sb3文件流
        const validate = require('./ada-school-parser/index.ts');
        const validationPromise = inputList.filter((item, idx) => (typeof step === 'number' ? step > idx : true)).map(
            input =>
                new Promise((resolve, reject) => {
                    this.fakeloadProject(input);
                    validate(input, false, (error, res) => {
        ...
        const validatedInput = await Promise.all(validationPromise);

        return validatedInput.map(([ input ], index) => {
            // 本地检测, 用定时模拟检测用时
            return this.checkTarget(input.targets);
        });
    }

    // 校验target
    checkTarget (right) {
        const left = [ ...this.runtime.targets ];
        const flagList = [];

        right.forEach(targetR => {
            // 背景
            if (targetR.isStage) {
                // 匹配对应背景
                const targetLIndex = left.findIndex(({ isStage }) => isStage);
                // 背景只有一个，找到匹配项后直接移除
                const targetL = left.splice(targetLIndex, 1)[0] || {};
                // const { assetId: assetIdL } = targetL.sprite.costumes[targetL.currentCostume];
                // const { assetId: assetIdR } = targetR.costumes[targetR.currentCostume];
                // !不再检查背景正确与否
                // if (assetIdL === assetIdR) {
                flagList.push(this.checkBlocks(targetL.blocks._blocks, targetR.blocks));
                // } else {
                //     console.error('背景错误');
                // }
            } else {
                ...
                for (let i = 0; i < left.length; i++) {
                    if (left[i].sprite.assetId === targetR.assetId) {
                        flagList.push(this.checkBlocks(left[i].blocks._blocks, targetR.blocks));
                    }
                }
            }
        });

        return flagList.length === right.length && !flagList.includes(false);
    }

    // 校验blocks
    checkBlocks (left, right) {
        // 反序列化json格式
        right = sb3.deserializeBlocks(right);

        const blocksLeft = Object.values(left);
        const blocksRight = Object.values(right).filter((item: any) => 'parent' in item && item.parent === null);

        // 已匹配
        const matchedDict = [];

        for (let idxR = 0; idxR < blocksRight.length; idxR++) {
            const blockR: any = blocksRight[idxR];
            for (let idxL = 0; idxL < blocksLeft.length; idxL++) {
                // 跳过已经匹配过的目标
                if (matchedDict.includes(idxL)) {
                    continue;
                }
                // 先判断opcode
                const blockL: any = blocksLeft[idxL];
                // opcode相同
                const flag = this.checkSingleBlock(blockL, blockR, left, right);
                if (flag) {
                    matchedDict.push(idxL);
                    continue;
                }
            }
        }

        return blocksRight.length === matchedDict.length;
    }

    // 对比两个单一模块是否一致
    checkSingleBlock (blockL, blockR, left, right): boolean {
        // 检查opcode是否一致
        if (blockR.opcode !== blockL.opcode) {
            console.error('checkSingleBlock: opcode不一致', blockL, blockR);
            return false;
        }

        // 检查fields是否一致
        if ('fields' in blockL && 'fields' in blockR) {
            const checkFieldsFlag = this.checkFields(blockL.fields, blockR.fields, 'value');
            if (!checkFieldsFlag) {
                console.error('checkSingleBlock: fields不一致', blockL, blockR);
                return false;
            }
        }

        // 检查inputs是否一致
        if ('inputs' in blockL && 'inputs' in blockR) {
            const checkInputsFlag = this.checkFields(blockL.inputs, blockR.inputs, 'name');
            if (!checkInputsFlag) {
                console.error('checkSingleBlock: inputs不一致（1)', blockL, blockR);
                return false;
            }

            const inputsL = Object.keys(blockL.inputs);
            const inputsR = Object.keys(blockR.inputs);

            if (inputsL.length !== inputsR.length) {
                console.error('checkSingleBlock: inputs不一致（2)', blockL, blockR);
                return false;
            }

            const inputCheckFlag = inputsL.map(inputKey => {
                const nextLeft = left[(blockL.inputs[inputKey] || {}).block];
                const nextRight = right[(blockR.inputs[inputKey] || {}).block];
                if (nextLeft && nextRight) {
                    return this.checkSingleBlock(nextLeft, nextRight, left, right);
                } else {
                    return false;
                }
            });
            ...
            // 包含input不一致
            if (inputCheckFlag.includes(false)) {
                console.error('checkSingleBlock: inputs不一致（3)', blockL, blockR);
                return false;
            }
        }

        if (blockR.next && !blockL.next) {
            return false;
        }

        if (blockR.next && blockL.next) {
            const nextLeft = left[blockL.next];
            const nextRight = right[blockR.next];
            return this.checkSingleBlock(nextLeft, nextRight, left, right);
        }
    ...
    // 检查两个对象的某一字段是否一致
    checkFields (obj1, obj2, attr): boolean {
        const target = Object.keys(obj2);

        let idx = 0;
        let rightKey = target[idx];
        let result = [];
        while (rightKey && obj1[rightKey]) {
            if (obj2[rightKey][attr] === obj1[rightKey][attr]) {
                result.push(true);
            } else {
                result.push(false);
            }
    ...
    saveProjectSb3 () {
        ...
        console.log('saveProjectSb3:', projectJson, this);
     ...
     */
    toJSON () {
        const resultJSON = sb3.serialize(this.runtime);
        console.log('调试字符串变量： 生成 sb3 作品，数据序列化前 this.runtime: ', this.runtime);
        console.log('调试字符串变量： 生成 sb3 作品，数据序列化前 resultJSON: ', resultJSON);

        return StringUtil.stringify(resultJSON);
    }
    ...
    deserializeProject (projectJSON, zip) {
        console.log('deserializeProject:', projectJSON);
        // Clear the current runtime
        this.clear();
        // 保存sb3文件格式
        this.runtime._projectJSON = projectJSON;
        ...
        // 解决 「Arduino 模块 sb3 文件报错」 核心代码
        // 注：函数 installDeviceExtensions 原本在函数 installDevice 内触发，修改为 函数 installTargets 之后触发。
        return (
            deserializePromise()
                ...
                // Question： cause error 的原因是？
                .then(targets => {
                    console.log('deserializePromise targets:', targets);
                    return this.installTargets(targets, projectJSON.extensions, true, !!projectJSON.device);
                })
    ...
    installDeviceExtensionsSync () {
        if (this.runtime._pendingDeviceExtensions) {
            ...
            return this.extensionManager
                .loadDeviceExtension(this.runtime._pendingDeviceExtensions.shift())
    ...
    installDeviceExtensions (deviceExtensions, targets) {
        if (!deviceExtensions || !deviceExtensions.length) {
            this.updateTargetsAndWorkspace(targets);

            console.log('installDeviceExtensions - 没有硬件', targets);
            return Promise.resolve();
        }

        console.log('installDeviceExtensions - 有硬件', deviceExtensions, targets);
    ...
    installDevice (
        ...
        // console.log('/ada-school-vm/src/virtual-machine - installDevice: targets', targets);
        // console.log('/ada-school-vm/src/virtual-machine - installDevice: device', device);
        // console.log('/ada-school-vm/src/virtual-machine - installDevice: deviceType', deviceType);
        // console.log('/ada-school-vm/src/virtual-machine - installDevice: pnpIdList', pnpIdList);
        targets = targets.filter(target => !!target);

        this.runtime.setRealtimeMode(programMode === 'realtime');
        if (device) {
            return this.extensionManager.loadDeviceURL(device, deviceType, pnpIdList).then(() => targets);
        }
        return targets;
    }
    ...
    installTargets (targets, extensions, wholeProject, hasDevice?) {
        console.log('installTargets:', targets, extensions, wholeProject, hasDevice);
        const allPromises = [];
        console.log('installTargets: extensions', extensions);
        if (extensions) {
            if (extensions.extensionIDs) {
                console.log('installTargets: extensions.extensionIDs', extensions.extensionIDs);
    ...
    addSprite (input) {
        ...
        const validationPromise = new Promise((resolve, reject) => {
            const validate = require('./ada-school-parser/index');
            ...
            validate(input, true, (error, res) => {
                console.log('addSprite:', input, res);
                if (error) return reject(error);
                resolve(res);
            });
        });
    ...
    _addSprite3 (sprite, zip) {
        console.log('_addSprite3:', sprite);
        // Validate & parse
        return sb3
    ...
    blockListener (e) {
        // 点击的是当前选中的积木
        if (this.runtime.currentBlockId && this.runtime.currentBlockId === e.blockId) {
            if (e.element === 'stackclick') {
                this.runtime.glowScript(this.runtime.currentBlockId, false);
                this.runtime.setCurrentBlockId(null);
                this.stopAll();
            }
            return;
        }
        if (this.runtime.currentBlockId && e.element !== 'stackclick') {
            // 有积木处于选中状态
            this.runtime.glowScript(this.runtime.currentBlockId, false);
            this.runtime.setCurrentBlockId(null);
            this.stopAll();
            ...
            if (!this.runtime.running) {
                if (e.element === 'stackclick') {
                    // 复制targets信息
                    this.runtime.setCurrentBlockId(e.blockId);
                    this.runtime.glowScript(e.blockId, true);
                    this.runtime.copyTargets();
                }
                if (this.editingTarget) {
                    // 运行
                    if (
                        this.editingTarget.blocks &&
                        this.editingTarget.blocks._blocks &&
                        this.editingTarget.blocks._blocks[e.blockId] &&
                        this.editingTarget.blocks._blocks[e.blockId].opcode &&
                        this._checkHatsArray.indexOf(this.editingTarget.blocks._blocks[e.blockId].opcode) > -1 &&
    ...
    // 清除选址你给的积木id
    clearCurrentBlockId () {
        this.runtime.clearCurrentBlockId();
    }
     ...
     * @param {boolean} isRealtimeMode 是否是实时模式
     */
    emitWorkspaceUpdate (isRealtimeMode?) {
        // console.log('实时/上传模式切换，创建变量 dom 节点，执行 emitWorkspaceUpdate 函数，是否 isRealtimeMode：', isRealtimeMode);
        ...
        let globalVariables = Object.keys(globalVarMap).map(k => globalVarMap[k]);
        let localVariables = Object.keys(localVarMap).map(k => localVarMap[k]);
        const workspaceComments = Object.keys(this.editingTarget.comments)
            .map(k => this.editingTarget.comments[k])
            .filter(c => c.blockId === null);

        // console.log('实时/上传模式切换，创建变量 dom 节点，执行 emitWorkspaceUpdate 函数，获取 globalVariables：', globalVariables);
        // console.log('实时/上传模式切换，创建变量 dom 节点，执行 emitWorkspaceUpdate 函数，获取 localVariables：', localVariables);

        if (isRealtimeMode === true || this.runtime.getCurrentIsRealtimeMode()) {
            // 实时模式不展示字符串变量：仅变量、列表变量
            globalVariables = globalVariables.filter(variable => variable.type !== Variable.STRING_VARIABLE_TYPE);
            localVariables = localVariables.filter(variable => variable.type !== Variable.STRING_VARIABLE_TYPE);
            // console.log('实时/上传模式切换，实时模式过滤掉字符串变量：', globalVariables);
            // console.log('实时/上传模式切换，实时模式过滤掉字符串变量：', localVariables);
        }
        if (isRealtimeMode === false || !this.runtime.getCurrentIsRealtimeMode()) {
            // 上传模式不展示列表变量：仅数字变量（及变量）、字符串变量，数字变量不展示默认变量
            globalVariables = globalVariables.filter(
                variable =>
                    variable.type !== Variable.LIST_VARIABLE_TYPE &&
                    (variable.type === Variable.SCALAR_VARIABLE_TYPE
                        ? variable.id !== '`jEk@4|i[#Fk?(8x)AV.-my variable'
                        : true),
            );

            localVariables = localVariables.filter(variable => variable.type !== Variable.LIST_VARIABLE_TYPE);
            // console.log('实时/上传模式切换，上传模式过滤掉列表变量：', globalVariables);
            // console.log('实时/上传模式切换，上传模式过滤掉列表变量：', localVariables);
        }
        ...
        // console.log('实时/上传模式切换，创建变量 dom 节点，执行 emitWorkspaceUpdate 函数，返回 xmlString：', xmlString);
    ...
    /**
     * postFaceInfo
     * @param {object} faceInfo 人脸识别数据
     */
    postFaceInfo (faceInfo) {
        this.runtime.emit('UPDATE_AI_DATA', faceInfo);
    }

    // 卡路里数据
    postCalorieInfo (calorieInfo) {
        this.runtime.emit('UPDATE_CALORIE_DATA', calorieInfo);
    }

    // 天气数据
    postWeatherInfo (weatherInfo) {
        this.runtime.emit('UPDATE_WEATHER_DATA', weatherInfo);
    }

    // 日期数据
    postDateInfo (dateInfo) {
        this.runtime.emit('UPDATE_DATE_DATA', dateInfo);
    }
